...
const SCHEMAS_TARGET_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..', 'schemas')
const TYPES_TARGET_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..', 'src', 'types', 'bom')
...
const bomConfig = {
  ...
  replacements: [
    [/,?\s*"format"\S*:\s*"string"/sg, ''],
    [/,?\s*"enum"\s*:\s*\[\s*"http:\/\/cyclonedx\.org\/schema\/.+?\.schema\.json"\s*\]/sg, ''],
...
const otherDownloads = [
  [`${SOURCE_ROOT_LATEST}spdx.schema.json`, join(SCHEMAS_TARGET_ROOT, 'spdx.schema.json')],
  [`${SOURCE_ROOT_LATEST}jsf-0.82.schema.json`, join(SCHEMAS_TARGET_ROOT, 'jsf-0.82.schema.json')],
  ['https://raw.githubusercontent.com/spdx/spdx-spec/refs/heads/support/2.3/schemas/spdx-schema.json', join(SCHEMAS_TARGET_ROOT, 'spdx-2.3.schema.json')],
  ['https://spdx.github.io/spdx-spec/v3.0.1/rdf/schema.json', join(SCHEMAS_TARGET_ROOT, 'spdx-3.0.1.schema.json')]
]
...
async function generateTypeScriptDefinitions(target, version) {
  try {
    ...
    const jsfSchemaContent = await readFile(join(SCHEMAS_TARGET_ROOT, 'jsf-0.82.schema.json'), 'utf8');
    const jsfSchema = JSON.parse(jsfSchemaContent);

    const spdxSchemaContent = await readFile(join(SCHEMAS_TARGET_ROOT, 'spdx.schema.json'), 'utf8');
    ...
    const schemaString = JSON.stringify(inlinedSchema)
      .replace(/"\$ref":\s*"jsf-0\.82\.schema\.json"/g, '"$ref": "#/definitions/signature"')
      .replace(/"\$ref":\s*"spdx\.schema\.json"/g, '"$ref": "#/definitions/spdxLicenseId"');
    ...
    const typeFilePath = target.replace('/schemas', '/src/types/bom').replace('.schema.json', '.schema.d.ts')
...
async function generateStandaloneTypeScriptDefinitions() {
  ...
  for (const { file, typeName } of standaloneSchemas) {
    try {
      ...
      const typeFilePath = schemaPath.replace('/schemas', '/src/types/bom').replace('.schema.json', '.schema.d.ts')
      await writeFile(typeFilePath, schemaType);
...
async function main() {
  ...
  const promises = bomConfig.versions.map(version => processBomSchema(version));
